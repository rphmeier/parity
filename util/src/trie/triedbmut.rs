...
///
/// Use it as a `TrieMut` trait object. This stores changes in-memory until
/// `commit` is called. Querying the root will indirectly call `commit`.
///
/// Use `db` and `db_mut` to access the underlying database.
///
/// # Example
/// ```
...
impl<'a> TrieDBMut<'a> {
	...
	/// Get a handle to the underlying db.
	pub fn db(&self) -> &HashDB { &*self.db }

	/// Get a mutable handle to the underlying db.
